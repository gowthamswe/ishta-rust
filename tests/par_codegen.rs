...
"#,
    ...
    fn frozen_param_emits_no_refcount_traffic_like_a_borrow() {
        let program = |mode: &str| {
            format!(
                "shared struct N {{ val: i64 }}\n\
                 fn inner(n: {mode}N) -> i64 {{ n.val }}\n\
                 fn outer(n: {mode}N) -> i64 {{ inner(n) }}\n\
                 fn main() {{ let g = N {{ val: 7 }}; println(outer(g)); }}"
            )
        };
        let traffic = |mode: &str| {
            let ir = ir_for_analyzed(&program(mode));
            (
                ir.matches("rc_inc").count(),
                ir.matches("rc_dec").count(),
                ir.matches("atomicrmw").count(),
            )
        };
...
"#,
    ...
    /// (`drop 5`) at parent scope exit, diverging from the sequential
    /// lanes. `Box2` carries no `impl Drop` itself, so the direct
    /// `drop_method_keys` gate missed it; the fixpoint closure over
    /// declared struct fields / enum payloads
    /// (`compute_drop_observable_types`) admits it via the `Res` payload
    /// and the captured mutation of `b` forces the group sequential. The
    /// expected `drop 5` right after the overwrite line is the sequential
    /// semantics: the B-2026-07-30-11 enum-assign displacement leg fires
    /// the overwritten payload's body at the assignment on every backend
    /// (this test originally pinned that position as a shared silence).
    #[test]
...
fn main() {
    ...
    #[test]
    fn test_e2e_auto_par_generic_tuple_param_element_is_owned_once() {
        let out = run_program(
            r#"
...
"#,
    ...
    /// measured 12 allocs / 10 frees, 11 B definitely lost in 2 blocks.
    ///
    ...
    /// generic TEMPLATE is never declared, so the table held no entry and the
    /// registrar declined. The result was left with NO owner at all:
    /// `call_arg_flows_into_return` had already stood the caller-side argument
    ...
    ///
    /// The auto-par column is a distinct surface for this family — an
    /// effect-analysis miss shows up here and nowhere else — and it carries
    /// the same one-body-per-object debt as the other three.
    #[test]
    fn test_e2e_auto_par_generic_whole_param_discarded_temp_runs_one_body() {
        let out = run_program(
            r#"
struct R { id: i64, tag: String, xs: Vec[i64] }
impl Drop for R { fn drop(mut ref self) { println(f"dR{self.id}") } }
...
"#,
    ...
    ///     still open.
    ///
    /// The auto-par column is its own surface for this family — the stand-down
    /// edits scope-cleanup frames and auto-par re-homes those into branch
    /// functions — and carries the identical debt.
    ///
    ...
    #[test]
    /// B-2026-09-06-52 — the AUTO-PAR twin: a whole rebind of a by-value param whose struct the
    ...
    /// cap-zeroing.
    fn test_e2e_auto_par_declined_copy_param_rebind_keeps_the_callers_ownership() {
        let out = run_program(
            r#"
shared struct Inner { v: i64 }
...
"#,
        ...
        if let Some(out) = out {
            assert_eq!(
                out, "inK\ndR82\na\ninK\nk88\ndR88\ninM\ndR89\nc\ninM\nm90\ndR90\ninN\ndR91\ne\ninS\ndR92\nf\ninK\ndDx93\ng\ninK\ndR94\nh\ninK\ndR95\ninK\ndR96\ninK\ndR97\nj\nend\n",
                "the auto-par column owes the same one body per object and the same \
                 free for a generic method's entry copy; got {out:?}"
            );
        }
    }
