...
| B-2026-09-05-22 | 2026-09-05 | runtime | medium | THE AUTO-PAR COLLAPSE IS AT EXACTLY TWO ACTIVE WORKERS, NOT A TWO-THREAD POOL: with iter_total=2 pinning n_workers=2, an allocating loop is 5.8-5.9x slower than N=1 at pool sizes 2, 3 AND 18 alike, while the same total work split THREE ways is faster than N=1 at every pool size (0.68-0.77x). At two workers the top-of-stack leaf is mach_absolute_time (211 samples) called from libsystem_malloc's contention path and ABSENT at three, with LESS real allocator work -- it is contending, not allocating. Two pthreads in C on the same pattern are 1.60x FASTER, so this is kara's | docs/investigations/autopar-alloc-scaling.md |
