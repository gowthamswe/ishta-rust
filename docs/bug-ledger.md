...
| B-2026-09-06-33 | 2026-09-06 | codegen | high | A PARTIAL STRUCT `let` PATTERN BINDS THE FIRST DECLARED FIELD REGARDLESS OF WHICH FIELD IS NAMED, and double-frees on the JIT -- `let s = S3 { a: mk(2), b: mk(3) }; let S3 { b, .. } = s; return b.id;` prints `dR2 dR2 dR1 v=2` under aot and `KARAC_AUTO_PAR=0` (`b` holds field `a`'s value, `a`'s body runs twice, `b`'s never) and aborts with `free(): double free detected in tcache 2` under jit; the interpreter reads `v=3` | — |
| B-2026-09-06-34 | 2026-09-06 | interp | medium | THE INTERPRETER NEVER RUNS THE `..` REST FIELDS' `Drop` BODIES IN A PARTIAL STRUCT `let` DESTRUCTURE -- `let s = S3 { a: mk(9), b: mk(10) }; let S3 { a, .. } = s; return a.id;` prints `dR9 dR8` under `--interp` against `dR10 dR9 dR8` on jit / aot / `KARAC_AUTO_PAR=0`; field `b` is moved out of `s` by the pattern and then owned by nobody | — |
...
| B-2026-09-06-37 | 2026-09-06 | interp+codegen | medium | A WILDCARD ARM OVER AN OWNED ENUM RECEIVER RUNS THE PAYLOAD'S `Drop` BODY ON NO SURFACE -- `impl E { fn m_none(self) -> i64 { match self { E.A(_) => { return 1; } E.B => { return 0; } } } }` prints `dE z1` for a named local and `z1` for a temp on --interp / jit / -O0 / -O2 alike; the payload's `dR` never runs, where the same arm binding the payload (`E.A(r) => r.id`) runs it once everywhere since B-2026-09-06-27 | — |
| B-2026-09-06-38 | 2026-09-06 | interp+codegen | low | A FRESH-TEMP OWNED ENUM RECEIVER LOSES THE ENUM SHELL'S OWN `Drop` BODY ON EVERY SURFACE -- `E.A(mk(2)).m_read()` prints `dR2 x2` and never `dE` on --interp / jit / -O0 / -O2, where the named-local receiver `let a = E.A(mk(1)); a.m_read()` prints `dR1 dE x1`; B-2026-09-04-30's receiver-temp registrar declines an enum receiver's bodies by design, and the arm channel runs only the payload it bound | — |
