...
| B-2026-09-10-1 | 2026-09-10 | codegen | medium | AN `Option[Vec[<heap-bearing struct>]]` PASSED AS AN ARGUMENT STILL DOUBLE FREES AFTER B-2026-09-09-22 -- `plainV(Some([S { s: f".." }, ..]))` aborts on every compiled backend while the SAME payload in a local option in one frame is clean, and the doubled block is the struct's 9-byte STRING field rather than an element buffer; a no-heap struct element is clean and a two-field one aborts, so it is the heap FIELD, not the single-field layout | none |
