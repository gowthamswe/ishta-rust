...
| B-2026-09-07-10 | 2026-09-07 | codegen+interp | high | A NAMED-LOCAL ARGUMENT TO A CALLEE THAT FORWARDS IT ONE HOP DOUBLE-FREES ON EVERY COMPILED BACKEND, AND THE INTERPRETER RUNS THE `Drop` BODY TWICE -- `let a = mk(20); let z = via(a);` over `fn via(r: R) -> R { return f(r); }` and `fn f(r: R) -> R { return r; }` aborts `free(): double free detected in tcache 2` under `karac run` and at both opt levels (invalid free + invalid read at -O2, plus an invalid write at -O0) over a struct with a `shared` field, while the FRESH-TEMP spelling of the same call is clean everywhere; `--interp` prints `dR20 c=20 dR20`, two bodies for one object, where the direct passthrough of the same object prints one | — |
| B-2026-09-07-11 | 2026-09-07 | codegen | high | A NAMED-LOCAL ARGUMENT TO A METHOD THAT STORES IT DOUBLE-FREES A DECLINED-COPY STRUCT -- `let a = mk(28); b.push(a);` over `impl Box2 { fn push(mut ref self, r: R) { self.xs.push(r); } }` aborts `free(): double free detected in tcache 2` on `karac run` and at BOTH opt levels (3 valgrind errors from 2 contexts), while the FREE-FUNCTION twin `let a = mk(46); take(mut b, a);` is correct on all five surfaces since B-2026-09-06-71's fix (6b21fe8) and the FRESH-TEMP spelling of both is correct since B-2026-09-07-5 | src/codegen/call_dispatch.rs (move_transferred_struct_arg, struct_param_transfer_eligible); method call path vs 6b21fe8 |
| B-2026-09-07-12 | 2026-09-07 | interp | medium | A NAMED-LOCAL ARGUMENT TO A STORING METHOD RUNS ITS `Drop` BODY TWICE UNDER `--interp`, IN BOTH COPY CLASSES, WHILE THE FREE-FUNCTION TWIN IS CORRECT -- `let a = mk(28); b.push(a);` prints `dR28 len=1 dR28` on the interpreter against `len=1 dR28` from the identical `take(mut b, a)` free function, and the copy-supported `let s = mks(45); b.add(s);` prints `dS45 len=1 dS45` on the interpreter while all four compiled surfaces print `len=1 dS45` correctly | src/interpreter/eval_call.rs (method arm vs free-function arm, named-argument move bookkeeping) |
