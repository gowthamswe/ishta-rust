...
| B-2026-09-09-22 | 2026-09-09 | codegen | medium | A `Vec[Vec[String]]` PAYLOAD BOUND OUT OF AN `Option` OR `Result` ARM DOUBLE FREES ON EVERY COMPILED BACKEND -- `match x { Some(t) => t[0][0] }` aborts with `free(): double free detected in tcache 2` at both opt levels and either way on auto-par while `--interp` prints the answer; the SAME payload in a user enum is clean, and so is the same envelope over `Vec[Vec[i64]]`, so it is the built-in envelope over a HEAP-BEARING element | none |
